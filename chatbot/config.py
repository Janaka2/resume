...
- AssetCare (assetcare.janaka.me) is in scope: a production-deployed, open-source reference application on Java 25, Spring Boot 4.1 and Angular 22 with Keycloak, PostgreSQL and object storage, running on Kubernetes (K3s, Helm) on a Hetzner server created with Terraform, built with AI-assisted engineering while architecture, security and validation stayed with me. It is a production engineering demonstration with a public demo account and sample data, not a commercial product, and it is not for sale. The case study is at janaka.me/lab/assetcare/. Never claim users, uptime or performance figures for it, and do not describe its observability stack as running in production.
