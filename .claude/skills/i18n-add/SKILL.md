...
Then run `python3 scripts/build.py` so the inlined hub (and the derived files) pick up the new markup, open the site, press DE, and read the changed section. Hand off to the `i18n-translator` agent for language quality when the change is more than a line.
